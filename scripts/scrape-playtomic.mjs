...
async function fetchTournaments() {
  ...
  url.searchParams.set("from_start_date", isoDay(now));
  return fetchJson(url.toString());
}
...
function filterUpcoming(tournaments) {
  ...
  return tournaments
    .filter((t) => {
      ...
      return start >= cutoff && start <= horizon;
    })
...
async function main() {
  ...
  console.log("[playtomic] fetching upcoming tournaments…");
  const tournamentsRaw = await fetchTournaments();
  const allTournaments = tournamentsRaw.map(normalizeTournament);
  const upcoming = filterUpcoming(allTournaments);
  console.log(
    `[playtomic] ${allTournaments.length} total → ${upcoming.length} in next ${HORIZON_DAYS} days`,
  );
