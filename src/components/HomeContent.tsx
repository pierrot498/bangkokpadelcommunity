...
export default function HomeContent({ locale }: { locale: Locale }) {
  ...
  const links: LinkItem[] = [
    {
      href: FACEBOOK_URL,
      label: d.home.facebookLabel,
      sublabel: d.home.facebookSublabel,
      primary: true,
      icon: fbIcon,
    },
    {
      href: INSTAGRAM_URL,
      label: d.home.instagramLabel,
      sublabel: d.home.instagramSublabel,
      icon: igIcon,
    },
  ];
  ...
  return (
    <>
      ...
      <section className="relative isolate flex flex-1 flex-col items-center justify-center px-5 py-12 sm:py-20">
        ...
        <article className="w-full max-w-md text-center">
          ...
          <nav aria-label={d.nav.home} className="mt-9 animate-fade-up">
            <h2 className="sr-only">{d.home.facebookLabel}</h2>
            <ul className="flex w-full flex-col gap-3">
              ...
              <li>
                <a
                  ...
                  className="group flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 px-5 py-4 text-left backdrop-blur-md transition-all hover:-translate-y-0.5 hover:border-[var(--accent)]/40"
                >
                  <span
                    aria-hidden="true"
                    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white/10 text-white group-hover:bg-[var(--accent)] group-hover:text-black"
                  >
                    <svg
                      viewBox="0 0 24 24"
                      className="h-6 w-6"
          ...
          <section aria-label="About" className="mt-10 text-balance text-sm text-white/55 animate-fade-up">
            <h2 className="sr-only">About Bangkok Padel Community</h2>
            <p>{d.home.about}</p>
            <p className="mt-2 text-white/40">
              <a className="underline hover:text-white" href={SITE_URL}>
                bangkokpadelcommunity.com
              </a>
            </p>
          </section>
        </article>
